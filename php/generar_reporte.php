...
// Función para obtener reservaciones pendientes
function obtenerReservaciones($tipoReserva) {
    ...
    $stmt = $conn->prepare("SELECT c.Nombre AS Nombre_Cliente, c.Apellido_Paterno AS Apellido_Paterno, r.Fecha_Reserva, r.Tipo_Reserva, r.Anticipo, r.Duracion, r.Total FROM Reservacion r INNER JOIN Cliente c ON r.ID_Cliente = c.ID_Cliente WHERE r.Fecha_Reserva > ? AND r.Tipo_Reserva = ?");
    ...
    while ($row = $result->fetch_assoc()) {
        $nombreCompleto = $row['Nombre_Cliente'] . ' ' . $row['Apellido_Paterno']; // Concatenar nombre y apellido paterno
        $pdf->Cell(40, 10, $nombreCompleto, 1, 0, 'C');
...
function obtenerReservacionesAnteriores($tipoReserva) {
    ...
    $pdf->SetFont('Arial', '', 10);
    while ($row = $result->fetch_assoc()) {
        $pdf->Cell(40, 10, $row['Nombre_Cliente'], 1, 0, 'C');
        $pdf->Cell(40, 10, $row['Fecha_Reserva'], 1, 0, 'C');
        $pdf->Cell(30, 10, $row['Tipo_Reserva'], 1, 0, 'C');
        $pdf->Cell(30, 10, $row['Anticipo'], 1, 0, 'C');
        $pdf->Cell(30, 10, $row['Duracion'] . ' hora(s)', 1, 0, 'C');
